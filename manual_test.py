...
# SENTINEL_FIX: Removed auto_init=True as it is now the default/only behavior
firewall = Sentinel()
...
class MockAgentAction:
    ...
    # Adding __repr__ for cleaner internal logging if needed
    def __repr__(self):
...
def execute_langchain_tool(action):
    print(f"[Tool Executed]: {action.tool}")
    return "Success"
...
# 'shell' tool is likely not in the 'restricted' role allowed list (which is empty by default)
